...
def main():
    ...
    readfile = 'ChargingPileLL.csv'
    with open(readfile, encoding="utf-8") as csvFile:
        ...
        for row in listReport:
            ...
            # slng = '121.431691'
            radius = '850' 
            searchdata = ['art_gallery', 'atm', 'bakery', 'bank', 'meal_takeaway', \
                          'beauty_salon', 'book_store', 'movie_theater', 'cafe', 'museum', \
                          'park', 'pet_store', 'pharmacy', 'church', 'clothing_store', \
                          'convenience_store', 'restaurant', 'doctor', 'drugstore', \
                          'shoe_store', 'shopping_mall', 'electronics_store', 'spa', \
                          'furniture_store', 'supermarket', 'gym', 'hair_care', \
                          'zoo', 'jewelry_store', 'laundry', 'tourist_attraction'
                         ]
            for s in range(len(searchdata)):
                ...
                for data in results:
                    ...
                    #欲存入路徑
                    db.collection(name).document(searchtype).collection('views').add(doc)
                wb.save(file)
                print('*'*70)
main()
